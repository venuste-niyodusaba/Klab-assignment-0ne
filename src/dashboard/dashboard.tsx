...
export default function Dashboard() {
  ...
  const fetchDashboard = async () => {
    ...
    try {
      const res = await axios.get<DashboardData>("https://kappee-backend-repo-10.onrender.com/api/dashboard", {
      ...
      if (axiosErr.response?.status === 403) {
        ...
        try {
          const tokenRes = await axios.post<{ token: string }>(
            "https://kappee-backend-repo-10.onrender.com/api/auth/refresh-token",
            { token: refreshToken }
          );
          localStorage.setItem("token", tokenRes.data.token);
          const retryRes = await axios.get<DashboardData>("https://kappee-backend-repo-10.onrender.com/api/dashboard", {
