...
export function Product() {
  ...
  const fetchProducts = async () => {
    try {
      const [productsRes, categoriesRes]: [AxiosResponse<ProductType[]>, AxiosResponse<CategoryType[]>] = 
        await Promise.all([
          axios.get<ProductType[]>("http://localhost:5175/api/products", { headers: { Authorization: `Bearer ${token}` } }),
          axios.get<CategoryType[]>("http://localhost:5175/api/categories", { headers: { Authorization: `Bearer ${token}` } })
        ]);
  ...
  const handleToggleActive = async (product: ProductType) => {
    try {
      const res: AxiosResponse<ProductType> = await axios.patch(
        `http://localhost:5175/api/products/${product._id}`,
  ...
  const handleSaveProduct = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (editingProduct) {
        res = await axios.patch(`http://localhost:5175/api/products/${editingProduct._id}`, dataToSend, { headers });
        setProducts(products.map((p) => (p._id === res.data._id ? res.data : p)));
        Notify.success("Product updated successfully");
      } else {
        res = await axios.post("http://localhost:5175/api/products", dataToSend, { headers });
