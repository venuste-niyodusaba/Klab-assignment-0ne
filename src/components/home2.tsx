      ...
      <div className="flex gap-6">
          ...
          <div className="relative border rounded p-4 flex flex-col items-center">
            ...
            <div className="bg-blue-500 flex flex-row px-14 py-3 justify-between gap-12">
              ...
              <div className="relative group/view">
                <Plus size={20} className="cursor-pointer hover:text-purple-600" />
                <span className="absolute -top-8 left-1/2 -translate-x-1/2 scale-0 group-hover/view:scale-100 transition-transform bg-black text-white text-xs px-2 py-1 pr-8">Quick View</span>
              </div>
            </div>
            <h3 className="mt-2 font-semibold">Apple Watch Series 5</h3>
            <p className="text-gray-600 text-sm">ELECTRONICS</p>
            <p className="mt-2"><span className="text-red-500 font-bold">$499.00</span> – <span className="text-gray-500 line-through">$599.00</span></p>
          </div>
        </div>
        ...
        <div className="flex-1 border shadow rounded p-4">
          <div className="flex justify-between items-center border-b pb-2 mb-4">
            ...
            <button className="text-sm font-semibold px-5 py-1 bg-black text-white rounded">VIEW ALL</button>
          </div>
          <div className="grid grid-cols-4 gap-6 w-[50rem]">
            {featuredProducts.map((product, i) => (
              <div key={product.id} className="relative border rounded shadow-md overflow-hidden cursor-pointer group transition-transform duration-300 hover:scale-105" onMouseEnter={() => setHoveredIndex(i)} onMouseLeave={() => setHoveredIndex(null)}>
                {product.badge && <span className="absolute top-2 left-2 bg-orange-500 text-white text-xs px-4 py-1 rounded">{product.badge}</span>}
                ...
                <div className="bg-blue-500 hidden group-hover:flex flex-row px-2 py-1 justify-between gap-10 transition-transform duration-300 hover:scale-105 rounded-lg shadow-md">
                  ...
                  <div className="relative group/compare">
                    <Shuffle size={20} className="cursor-pointer hover:text-blue-600" />
                  </div>
                  <div className="relative group/view">
                    <Plus size={20} className="cursor-pointer hover:text-purple-600" />
                  </div>
      ...
      <div className="border shadow rounded p-4">
        ...
        <div className="grid grid-cols-3 gap-6">
          {products.map((product, i) => (
            <div key={product.id} className="relative border rounded shadow-md bg-white p-4 flex flex-col items-center text-center transition-transform duration-300 hover:scale-105" onMouseEnter={() => setHoveredIndex(i)} onMouseLeave={() => setHoveredIndex(null)}>
              {product.badge && <span className={`absolute top-2 left-2 text-white text-xs px-2 py-1 rounded ${product.badge.includes("%") ? "bg-green-600" : "bg-orange-500"}`}>{product.badge}</span>}
              <button className="absolute top-2 right-2 text-gray-500 hover:text-red-500"><Heart size={16} /></button>
              <div className="relative w-full h-36 flex items-center justify-center">
                <img src={hoveredIndex === i && product.back ? product.back : product.front} alt={product.title} className="object-contain max-h-full transition-all duration-500" />
              </div>
